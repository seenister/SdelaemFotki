...
class ERForms_Frontend {
    ...
    private function register_scripts() {
        wp_register_script('erf-global',ERFORMS_PLUGIN_URL.'assets/js/erforms-global.js',array('jquery'),ERFORMS_VERSION);
                    ...
                    foreach ($user_field_map as $req_key => $meta_key) {
                        ...
                        if (isset($data[$req_key]) && !$is_primary_key) {
                            ...
                            foreach($m_keys as $m_key){
                                if(!empty($m_key)){
                                    $status = update_user_meta($id, $m_key, $data[$req_key]);
            ...
            if (!empty($user->ID)) {
                ...
                foreach ($user_field_map as $req_key => $meta_key) {
                    if (isset($data[$req_key])) {
                        ...
                        foreach($m_keys as $m_key){
                            if(!empty($m_key)){
                                $status= update_user_meta($user->ID, $m_key, $data[$req_key]);
