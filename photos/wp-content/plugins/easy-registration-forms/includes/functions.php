...
function erforms_text_helpers() {
    $help_texts = array(
        ...
        'dataRefLabel' => __('This is for refering non input fields (For internal purpose) and will not visible on front end.','erforms'),
...
function erforms_system_notification_by_type($type,$options){
    $notifications= array();
    switch($type){
        case 'offline': $notifications= array('enabled'=>$options['send_offline_email'],'subject'=>$options['offline_email_subject'],'help'=>__('Sends email to users to let them know about the payment procedure.','erforms')); break;
        case 'payment_pending': $notifications= array('enabled'=>$options['en_payment_pending_email'],'subject'=>$options['pending_pay_email_subject'],'help'=>__('Email will be sent to user when payment status is pending.','erforms')); break;
        case 'payment_completed': $notifications= array('enabled'=>$options['en_payment_completed_email'],'subject'=>$options['completed_pay_email_subject'],'help'=>__('Email will be sent to user when payment status is completed.','erforms')); break;
        case 'payment_completed_admin': $notifications= array('enabled'=>$options['en_pay_completed_admin_email'],'subject'=>$options['completed_pay_admin_email_subject'],'help'=>__('Email will be sent to Admin when payment status is completed.','erforms')); break;
        case 'note_to_user': $notifications= array('enabled'=>$options['en_note_user_email'],'subject'=>$options['note_user_email_subject'],'help'=>__('Email will be sent to user when admin adds any note within the submission and check "notify user" option.','erforms')); break;
        case 'forgot_password': $notifications= array('enabled'=>1,'subject'=>$options['forgot_pass_email_subject'],'help'=>__('Forgot password email content.','erforms')); break;
    }
...
function erforms_array_field_sanitizer(&$item, $key){
...
function erforms_short_tags($form,$submission){
    ...
    if(!empty($submission['user'])){
        ...
        $short_tags['{{display_name}}'] = $submission['user']['display_name'];
    }
